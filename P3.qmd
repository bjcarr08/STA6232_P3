...
newData <- data.frame(HTN = rep(c("Normotensive", "Hypertensive"), each = 3*ageN),
                      HDL = rep(rep(c("Low","Normal","High"), 2), each = ageN),
                      age = rep(ageMin:ageMax, 6),
                      weight = rep(weightMedian, 6*ageN))

newData <- cbind(newData, predict(m3, newdata = newData, type = "probs"))
...
newData %>%
  ...
  ggplot(aes(x=age, y=Probability, color=HDL)) + 
